...
export const handleSubscribe = async (request, env, ctx, origin) => {
  ...
  try {
    ...
    // Sync to Systeme.io — tags the contact so a Systeme.io automation
    // can deliver the checklist PDF and start a nurture sequence.
    ctx.waitUntil(
      syncToSystemeIo(env, {
        email: validation.data.email,
        tagName: 'Lead Magnet - Website Checklist',
      })
    );
