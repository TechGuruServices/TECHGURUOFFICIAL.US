...
import { syncToSystemeIo } from './systemeio';
...
export const handleContact = async (request, env, ctx, origin) => {
  ...
  try {
    ...
    // Sync to Systeme.io — tags the contact "Contact Form Lead" so a
    // Systeme.io automation can follow up automatically.
    ctx.waitUntil(
      syncToSystemeIo(env, {
        email: validation.data.email,
        firstName: validation.data.name.split(' ')[0],
        tagName: 'Contact Form Lead',
      })
    );
